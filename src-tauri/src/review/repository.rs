...
pub fn replace_files(
    ...
    transaction
        .execute(
            "DELETE FROM review_files WHERE session_id = ?1",
            [session_id],
        )
...
pub fn mark_session_cancelled(database: &AppDatabase, session_id: &str) -> Result<(), String> {
    let connection = database.connect()?;
    let now = now_millis();
    connection
        .execute(
            "UPDATE review_sessions
             SET status='cancelled', phase='cancelled', updated_at=?2, completed_at=?2
             WHERE id=?1 AND status='running'",
            rusqlite::params![session_id, now],
        )
        .map_err(|error| format!("标记 Review 已取消失败: {error}"))?;
    Ok(())
}
        ...
        .query_row(
            ...
            |row| {
                Ok(ReviewSession {
                    id: row.get(0)?,
                    repo_root: row.get(1)?,
                    diff_fingerprint: row.get(2)?,
                    status: row.get(3)?,
                    phase: row.get(4)?,
                    progress_done: row.get(5)?,
                    progress_total: row.get(6)?,
                    current_file: row.get(7)?,
                    budget_mode: row.get(8)?,
                    model_id: row.get(9)?,
                    selected_files: from_json(row.get::<_, String>(10)?)?,
                    overview: from_json(row.get::<_, String>(11)?)?,
                    limitations: from_json(row.get::<_, String>(12)?)?,
                    input_tokens: row.get(13)?,
                    output_tokens: row.get(14)?,
                    usage_estimated: row.get(15)?,
                    error_message: row.get(16)?,
                    created_at: row.get(17)?,
                    updated_at: row.get(18)?,
                    completed_at: row.get(19)?,
                    is_pinned: row.get(20)?,
                    files: Vec::new(),
                    findings: Vec::new(),
                    ai_calls: Vec::new(),
                })
    ...
    let rows = statement
        .query_map(params![repo_root, limit], |row| {
            ...
            Ok(ReviewSessionSummary {
                ...
                status: row.get(2)?,
                phase: row.get(3)?,
                selected_file_count: selected.len(),
                overview: from_json(row.get::<_, String>(5)?)?,
                input_tokens: row.get(6)?,
                output_tokens: row.get(7)?,
                usage_estimated: row.get(8)?,
...
pub fn list_rules(database: &AppDatabase) -> Result<Vec<ReviewRule>, String> {
    ...
    let rows = statement
        .query_map([], |row| {
            Ok(ReviewRule {
                ...
                severity: row.get(5)?,
                category: row.get(6)?,
                include_globs: from_json(row.get::<_, String>(7)?)?,
                exclude_globs: from_json(row.get::<_, String>(8)?)?,
                languages: from_json(row.get::<_, String>(9)?)?,
                definition: from_json(row.get::<_, String>(10)?)?,
    ...
    rows.collect::<Result<Vec<_>, _>>()
        .map_err(|error| format!("读取 Review 规则失败: {error}"))
}

pub fn delete_rule(database: &AppDatabase, id: &str) -> Result<bool, String> {
    let connection = database.connect()?;
    connection
        .execute("DELETE FROM review_rules WHERE id=?1", [id])
        .map(|count| count > 0)
        .map_err(|error| format!("删除 Review 规则失败: {error}"))
}
...
fn load_files(
    ...
    session_id: &str,
) -> Result<Vec<ReviewFileRecord>, String> {
    ...
    let rows = statement
        .query_map([session_id], |row| {
            Ok(ReviewFileRecord {
                ...
                score_categories: from_json(row.get::<_, String>(3)?)?,
                score_breakdown: from_json(row.get::<_, String>(4)?)?,
                selected: row.get(5)?,
                review_status: row.get(6)?,
                batch_id: row.get(7)?,
                limitation: row.get(8)?,
            })
        })
    ...
    let rows = statement
        .query_map([session_id], |row| {
            Ok(ReviewFinding {
                ...
                problem: row.get(11)?,
                impact: row.get(12)?,
                trigger_scenario: row.get(13)?,
                evidence: row.get(14)?,
                suggestion: row.get(15)?,
                verified: row.get(16)?,
                status: row.get(17)?,
...
fn load_ai_calls(
    ...
    let rows = statement
        .query_map([session_id], |row| {
            Ok(AiCallUsage {
                batch_id: row.get(0)?,
                files: from_json(row.get::<_, String>(1)?)?,
...
fn validate_rule(rule: &ReviewRule) -> Result<(), String> {
    if rule.id.trim().is_empty() || rule.name.trim().is_empty() {
        return Err("规则 id 和名称不能为空".to_string());
    }
    if !matches!(rule.kind.as_str(), "deterministic" | "semantic") {
        return Err("规则类型必须是 deterministic 或 semantic".to_string());
    }
    if rule.kind == "semantic" && rule.definition.to_string().chars().count() > 600 {
        return Err("语义规则过长，请收窄到 300 字符左右".to_string());
    }
    Ok(())
}
...
#[cfg(test)]
mod tests {
    use super::mark_session_cancelled;
    use crate::storage::{migrations::run_migrations, AppDatabase};
    use rusqlite::params;
    use std::time::{SystemTime, UNIX_EPOCH};

    #[test]
    fn cancelling_a_review_preserves_existing_results_and_usage() {
        let nanos = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("system time")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("halowake-review-cancel-{nanos}"));
        std::fs::create_dir_all(&dir).expect("create test directory");
        let database = AppDatabase::from_path(dir.join("test.db"));
        ...
        connection
            .execute(
                "INSERT INTO review_sessions (
                    id, repo_root, diff_fingerprint, status, phase, budget_mode, model_id,
                    selected_files_json, overview_json, limitations_json, input_tokens,
                    output_tokens, usage_estimated, created_at, updated_at
                 ) VALUES (?1, '.', 'fingerprint', 'running', 'ai-review', 'balanced', 'model',
                    '[]', ?2, ?3, 120, 30, 0, 1, 1)",
                params![
                    "review-cancel-test",
                    r#"{"critical":1,"major":2,"minor":0,"suggestion":0,"appliedRules":1,"triggeredRules":1}"#,
                    r#"["partial result"]"#
                ],
            )
        ...
        mark_session_cancelled(&database, "review-cancel-test").expect("cancel session");

        let connection = database.connect().expect("reconnect");
        let state = connection
            .query_row(
                "SELECT status, phase, overview_json, limitations_json, input_tokens, output_tokens
                 FROM review_sessions WHERE id='review-cancel-test'",
                [],
                |row| {
                    Ok((
                        row.get::<_, String>(0)?,
        ...
        assert!(state.2.contains("\"critical\":1"));
        assert_eq!(state.3, r#"["partial result"]"#);
        assert_eq!(state.4, 120);
        assert_eq!(state.5, 30);
    }
}
