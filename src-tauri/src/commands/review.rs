...
use crate::{
    review::{
        models::{
            ReviewRule, ReviewSession, ReviewSessionSummary, StartReviewPayload,
...
pub async fn start_local_code_review(
    ...
    let plan = tokio::task::spawn_blocking(move || {
        planner::build_plan(
            &plan_db,
            &plan_payload.repo_path,
            &plan_payload.selected_files,
            plan_payload.budget_mode,
        )
    })
    .await
    .map_err(|error| format!("创建 Review 计划任务失败: {error}"))??;
    let session_id = format!(
        "review-{}-{}",
        repository::now_millis(),
        REVIEW_ID_COUNTER.fetch_add(1, Ordering::Relaxed)
    );
    repository::create_session(
        ...
        &plan.repo_root,
        &plan.fingerprint,
        payload.budget_mode.as_str(),
        &payload.model.id,
        &payload.selected_files,
        &plan.rules,
        crate::commands::storage::current_retention_days(&app),
    )?;
...
#[tauri::command]
pub fn list_local_code_reviews(
    database: State<'_, AppDatabase>,
    repo_root: String,
    limit: Option<usize>,
...
pub fn cancel_local_code_review(
    ...
    if cancelled {
        repository::mark_session_cancelled(&database, &session_id)?;
    }
    Ok(cancelled)
}
...
#[tauri::command]
pub fn list_review_rules(database: State<'_, AppDatabase>) -> Result<Vec<ReviewRule>, String> {
    repository::list_rules(&database)
}

#[tauri::command]
pub fn save_review_rule(
    database: State<'_, AppDatabase>,
    rule: ReviewRule,
) -> Result<Vec<ReviewRule>, String> {
    repository::upsert_rule(&database, &rule)?;
    repository::list_rules(&database)
}

#[tauri::command]
pub fn delete_review_rule(
    database: State<'_, AppDatabase>,
    id: String,
) -> Result<Vec<ReviewRule>, String> {
